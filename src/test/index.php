		...
		<div class="prices__models">
				<?
					...
					while ($device = $get_devices_data-> fetch_assoc()) {
						if ($counter >= 10) {
							$is_hidden = 'prices__model_hidden';
						}
				?>
					<div class="prices__model <?=$is_hidden?>" data-device="<?= $device['DEVICE_NAME']?>">
						<picture>
							<source srcset="./assets/images/webp/<?= $device['DEVICE_URL']?>.webp" type="image/webp">
							<img src="./assets/images/<?= $device['DEVICE_URL']?>.png" alt="<?= $device['DEVICE_NAME']?>" class="prices__model-img">
						</picture>
